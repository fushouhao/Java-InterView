...
统计每个同学的数学成绩 如果>85 优秀 >60 及格 否则不及格
### ***9.约束***
<img width="346" alt="image" src="https://github.com/fushouhao/Java-InterView/assets/94434988/2edc0b3a-0209-4beb-96ec-f7afc6ca4039">

###
外键
<img width="398" alt="image" src="https://github.com/fushouhao/Java-InterView/assets/94434988/12263b7c-b9a2-49b3-83c9-ae0cc0a7c62b">
<img width="422" alt="image" src="https://github.com/fushouhao/Java-InterView/assets/94434988/6bd616af-852c-4459-91fb-7f51a7d9e7ac">
### ***10.多表关系***
1.一对多
多的一方建立外键
2.多对多
建立一个中间表 至少包含两个外键 分别关联相关主键
3.一对一 类似 用户基本信息 和用户详细信息-----》单表拆分
在任意一方加入外键 关联另一方的主键 并且设置外键为唯一的UNIQUE
### ***11.多表查询***
### ****1.内连接****
<img width="314" alt="image" src="https://github.com/fushouhao/Java-InterView/assets/94434988/d421ee6e-105f-48ba-993e-d92e6a5bd4a2">
...
### ****3.联合查询****
列数不同不能合并
Union all 将两个查询的结果合为一处
Union 在合并的基础上去重
### ****4.子查询****
1.标量子查询 子查询返回的结果是单个值
2.列子查询 返回的结果是一列
<img width="407" alt="image" src="https://github.com/fushouhao/Java-InterView/assets/94434988/7c16cd15-c906-45f9-bb24-e072bbfc80cf">

比财务部所有人工资都高的员工信息
比研发不其中任意一人工资高的员工信息
3.行子查询
<img width="235" alt="image" src="https://github.com/fushouhao/Java-InterView/assets/94434988/3cbfc0b7-a14f-47d1-9955-3c9e73c8458c">
4.表子查询
### ****5.事务****
### ***** 1.ACID 特性 *****
   A 原子性 要么都完成 要么都不动
   ...
   I 隔离性 事务之间不被干扰
   ###
   D 持久性 一个事务被提交之后,它对数据库中数据的改变是持久的,及时数据库发生故障也不应该对其有任何影响.
   ###
### ***** 2.并发事务 *****
     更新丢失 A事务撤销 把已经提交的B事务覆盖
   ###
比如在我们项目中，我们一个一段的逻辑是先寻找数据库里的旧数据，将旧数据清除，然后在清除的基础上，插入上游发送的数据。但是最开始由于上游发的是单线程数据，并且发送频率很慢，所以没有引入事务，但是最近上游为了提高效率 改成多线程发送数据，这样就会造成数据出现问题，所以首先引入事务，将插入和删除操作绑定为一个事务 保证这段业务的原子性，然后在这个事务基础上可能会出现更新丢失的情况首先在业务层面，进行了调整，首先先根据部分主键查询所有符合这些主键的数据，将这些数据放入集合里，然后比较上游发的数据，如果主键之间不完全一致再删除，并且判断 如果数据完全一致 则不进行插入操作。
其次在代码层面 对插入删除操作加了一个同步锁，并且把这两个操作作为同一事务 使用 REQUIRES_NEW：新建事务，如果当前存在事务，把当前事务挂起 考虑到加同步锁可能会造成线程死锁，
###
对代码进行进一步的更新，如果这个线程没有抢到当前资源，那么则需要休眠1000ms 如果连续五次休眠 则会提示这条数据被锁住，并且结束这个线程
 ###     
   脏读 dirty read：比如存款和取款同时进行 当取款操作未提交时存款操作读到了取款后的余额，但是当取款操作撤销之后，存款操作的余额仍然是撤销之前的余额，导致读了脏数据 -- 存100 取500 余额1000的状态下 先取500 再存100 但是 取完500后又撤销了而存款是基于取完500的余额基础上存100，这500 就是脏数据，最后存款后余额变成600，这就是脏读 本质上是读取到其他事务未提交的数据
   B事务读到了A事务没提交的数据 在这个期间内A事务可能会因为update的不规范 而终止事务 导致这个数据库的数据并没有按照A事务的要求去改变 但是B事务读到了这个数据 
 ###  
     不可重复读 unrepeatable read 在一个事务中 读取两次相同数据时 结果不同
同时进行取款操作 ，事务一先查询余额 1000元 事务2 查询余额后直接取款100并提交 
当事务一再次读取余额的时候 余额就变成了900元 
  A事务由于B事务在中间过程更新导致同一条数据前后两次的结果不一样 
###
     幻读 phantom 一般发生在计算统计数据的事务中 ，假设银行系统在同一事务中两次统计存款的总金额 在两次统计的过程中 刚好新增一个存款账户 并存入100元 这时两次统计的总金额将不一致
   查询时没查到 插入时告诉有这条数据  
### ***** 3.事务的隔离级别 *****
